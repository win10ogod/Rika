...
成就不是通用工具清單的換皮，而是理華與作者關係的可見時間線。角色載入會主動同步自己的成就目錄，快取失效時也會重新註冊，不會把不存在的成就 404 靜默略過。
...
`OwnerUserID` 必須是 Telegram 的純數字 user ID（例如 `123456789`），不能填 `@username`；理華會在 bot 啟動前拒絕錯誤格式，避免顯示登入成功後卻把主人私訊當成一般訊息。`OwnerUserName` 填不含 `@` 的帳號名稱，不是可含空格的顯示名稱；`OwnerNameKeywords` 則填作者的稱呼與別名，而不是理華自己的名字。
