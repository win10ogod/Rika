...
import { is_dist, charname as BotCharname, username as FountUsername, Rika } from '../charbase.mjs'

import { sendAndLogReply } from './reply.mjs'
import { errorRecord, userIdToNameMap } from './state.mjs'
...
`),
		...
		return await Rika.interfaces.chat.GetReply(selfRepairRequest)
	}
	catch (anotherError) {
		...
		if (`${error.name}: ${error.message}` === `${anotherError.name}: ${anotherError.message}`)
			return { content: '沒有足夠資訊產生可靠的修復方案。' }
