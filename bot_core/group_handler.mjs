...
import { charname as BotCharname, username as FountUsername, Rika } from '../charbase.mjs'
 ...
 * @param {string | number | null} ownerOverride - (可选) 测试用的使用者用户 ID 覆盖。
 * @returns {Promise<boolean>} 如果使用者存在则返回 true，否则返回 false。
 */
async function checkOwnerPresence(group, platformAPI, ownerOverride = null) {
	...
	return true // 如果检查失败，默认使用者存在（防止网络波动时乱退群）
}
...
async function generateInsult(group, platformAPI, defaultChannel, channelHistoryForAI) {
	...
	const insultSystemPrompt = `你目前在 ${groupNameForAI} 群組中，但這裡沒有找到設定的管理使用者。
用理華冷淡、簡短的語氣說明你將自動退出群組。
`
