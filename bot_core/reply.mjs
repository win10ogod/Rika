...
import { channelChatLogs, channelLastSendMessageTime, channelCharScopedMemory, userIdToNameMap, bannedStrings, fuyanMode } from './state.mjs'
...
export async function doMessageReply(triggerMessage, platformAPI, channelId) {
	...
	try {
		...
		const aiFinalReply = fuyanMode ? { content: '收到。' } : await Rika.interfaces.chat.GetReply(replyRequest)
