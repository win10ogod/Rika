...
import { channelLastSendMessageTime, channelMuteStartTimes, currentConfig, fuyanMode, setFuyanMode, bannedStrings, channelChatLogs, RikaWords, repeatBlacklist, waitForOwnerTypingToEnd, clearOwnerTypingStartTime } from './state.mjs'
 ...
 * @param {{has_other_rika_bot?: boolean}} [env={}] - 环境信息对象，例如是否检测到其他机器人。
...
function calculateKeywordBasedScore(content) {
	...
	const keywordMappings = [
		{ keywords: [/(有点|好)紧张/, '救救', '帮帮', '帮我', '来人', '咋用', '教教', /是真的(吗|么)/], score: 100 },
...
async function calculateOwnerTriggerIncrement(fountEntry, content, platformAPI, channelId, possible, isInFavor, mentionedWithoutAt) {
	...
	if (isInFavor && base_match_keys(content, ['闭嘴', '安静', '肃静']) && content.length < 10) {
		...
		return { newPossible: 0, isMutedUpdate: isMutedChannelUpdate } // 如果使用者静音则提前退出
	}
 ...
 * @param {string | number} channelId - 频道 ID。
 * @param {string} content - 清理后的消息内容。
 * @param {{has_other_rika_bot?: boolean}} [env={}] - 环境信息。
...
async function calculateTriggerPossibility(fountEntry, platformAPI, channelId, content, env = {}) {
	...
	const isChineseNamePattern = base_match_keys(contentEdgesForChineseCheck, ['理華', '理华'])
	...
	// Check for phrases that negate a direct mention (e.g., "理華的", "rika's")
	const isPossessiveOrStatePhrase = base_match_keys(content, [
		/(理華(有(?!没有)|能|这边|目前|[^ 。你，]{0,3}的)|(rika('s|is|are|can|has)))/i
	])
	// Check if the bot's name is at the very end of a short sentence, which might not be a direct mention.
	const isNameAtEndOfShortPhrase = base_match_keys(content, [/^.{0,4}理華$/i])

	const mentionedWithoutAt = !env.has_other_rika_bot &&
	...
	possible += base_match_keys(content, RikaWords) * 5
	...
	if (isFromOwner) {
		...
		if (ownerResult.isMutedUpdate)  // 如果使用者静音了频道
			return { possibility: 0, isMutedChannel: true, mentionedWithoutAt }

		// 如果使用者取消了静音，isMutedChannel 可能已变为 false，若未被使用者明确静音，则根据全局状态重新评估
 ...
 * @param {boolean} isFromOwner - 是否来自使用者。
...
function logTriggerCheckDetails(fountEntry, platformAPI, channelId, content, possible, okey, isFromOwner, mentionedWithoutAt, env = {}) {
	...
	console.dir({
		...
		mentionedWithoutAt,
		hasOtherRikaBot: env.has_other_rika_bot,
	}, { depth: null })
}
...
async function handleOwnerCommandsInQueue(currentMessageToProcess, platformAPI, channelId) {
	if (currentMessageToProcess.extension?.is_from_owner) {
		...
		{
			if (base_match_keys(content, [/^理華.{0,2}敷衍点.{0,2}$/])) setFuyanMode(true)
			if (base_match_keys(content, [/^理華.{0,2}不敷衍点.{0,2}$/])) setFuyanMode(false)
			if (base_match_keys(content, [/^理華.{0,2}(捂住耳朵|关上耳朵|[关闭]耳|别听|关闭听觉|中断听觉).{0,2}$/])) {
				await setMyData({ reality_channel_disables: { voice_sentinel: true } })
				const replyContent = '聲音偵測已關閉。'
			...
			if (base_match_keys(content, [/^理華.{0,2}(可以听了|张开耳朵|开耳|开启听觉|恢复听觉).{0,2}$/])) {
				await setMyData({ reality_channel_disables: { voice_sentinel: false } })
				const replyContent = '聲音偵測已開啟。'
				await sendAndLogReply({ content: replyContent }, platformAPI, channelId, currentMessageToProcess)
				newUserMessage(content, platformAPI.name)
				newCharReply(replyContent, platformAPI.name)
				return TriggerResultType.HANDLED
			}
			if (base_match_keys(content, [/^理華.{0,2}自裁.{0,2}$/])) {
				const selfDestructReply = { content: '正在退出。' }
			...
			const repeatMatch = content.match(/^理華.{0,2}复诵.{0,2}\s*(?<backticks>`+)[^\n]*\n(?<repeat_content>[\S\s]*?)\k<backticks>\s*$/)
			...
			const banWordMatch = content.match(/^理華.{0,2}禁止.{0,2}`(?<banned_content>[\S\s]*)`$/)
			if (banWordMatch?.groups?.banned_content)
				bannedStrings.push(banWordMatch.groups.banned_content)

		}
	}
	return TriggerResultType.NO_ACTION // 没有处理使用者命令或消息非来自使用者
}
...
async function checkQueueMessageTrigger(currentMessageToProcess, currentChannelLog, platformAPI, channelId) {
	...
	if (await checkMessageTrigger(currentMessageToProcess, platformAPI, channelId, { has_other_rika_bot: hasOtherRikaBot }))
		...
		return TriggerResultType.TRIGGER_REPLY // 如果是使用者在仅有使用者和机器人的对话中发言，则触发
	else if (
		!isMutedChannel &&
		...
		if (
			...
			!base_match_keys(repeat.element.content, [...currentMessageToProcess.extension.OwnerNameKeywords || [], ...rude_words, ...RikaWords, ...repeatBlacklist]) &&
			...
			if (
				(repeat.element?.content || repeat.element?.files?.length) &&
				repeat.count >= currentConfig.RepetitionTriggerCount &&
				!base_match_keys(repeat.element.content + '\n' + (repeat.element.files || []).map(file => file.name).join('\n'), [...currentMessageToProcess.extension.OwnerNameKeywords || [], ...rude_words, ...RikaWords, ...repeatBlacklist]) &&
				!isBotCommand(repeat.element.content) &&
