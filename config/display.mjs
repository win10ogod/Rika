...
const saveFile = async (fileName, blob, statusElement) => {
	...
	statusElement.textContent = geti18n('Rika.config.saving')
	try {
		...
		if (!response.ok) {
			...
			throw new Error(error.message || geti18n('Rika.config.failed_to_save_file_generic'))
		}
		const result = await response.json()
		statusElement.textContent = geti18n('Rika.config.file_saved', { fileName })
		console.log('File saved successfully', result)
		return true
	}
	catch (error) {
		console.error('Error saving file:', error)
		statusElement.textContent = geti18n('Rika.config.save_failed', { errorMessage: error.message })
		return false
	}
}
...
const saveRawAudio = async (fileName, audioBuffer, statusElement) => {
	...
	try {
		...
		if (!response.ok) {
			const error = await response.json()
			throw new Error(error.message || geti18n('Rika.config.failed_to_save_audio_file_generic'))
		}
		const result = await response.json()
		statusElement.textContent = geti18n('Rika.config.file_saved', { fileName })
...
async function handleRecorderStop() {
	recordingStatus.textContent = geti18n('Rika.config.processing_recording')
	try {
		const webmBlob = new Blob(audioChunks, { type: 'audio/webm' })
		if (!webmBlob.size) throw new Error(geti18n('Rika.config.empty_recording_file'))
		...
		if (success) {
			...
			audioStatus.textContent = geti18n('Rika.config.new_reference_voice_saved_and_loaded')
		}
	}
	catch (error) {
		console.error('Error processing or saving recording:', error)
		recordingStatus.textContent = geti18n('Rika.config.processing_failed', { errorMessage: error.message })
	}
	finally {
		audioChunks = []
		if (audioStream)
...
const loadPreview = async (filePath, element, statusElement, type) => {
	statusElement.textContent = geti18n('Rika.config.loading_preview')
	try {
		...
		if (response.ok) {
			...
			if (type === 'audio') {
				...
				statusElement.textContent = geti18n('Rika.config.preview_loaded')
			}
			else if (type === 'photo') {
				element.classList.add('hidden') // Keep it hidden by default
				statusElement.textContent = geti18n('Rika.config.reference_photo_set_click_to_toggle')
			...
			else statusElement.textContent = geti18n('Rika.config.preview_loaded')

		}
		else if (response.status === 404) {
			statusElement.textContent = geti18n('Rika.config.no_reference_file_set')
			if (type === 'photo') {
		...
		statusElement.textContent = geti18n('Rika.config.load_preview_failed', { errorMessage: error.message })
	}
}
...
const resampleAudio = async webmBlob => {
	try {
		...
		throw new Error(geti18n('Rika.config.audio_processing_failed', { errorMessage: error.message }))
	}
}
...
async function handleRecordButtonClick() {
	try {
		...
		recordingStatus.textContent = geti18n('Rika.config.recording_in_progress')
	}
	catch (error) {
		console.error('Error starting recording:', error)
		recordingStatus.textContent = geti18n('Rika.config.cannot_start_recording', { errorMessage: error.message })
		if (audioStream)
			audioStream.getTracks().forEach(track => track.stop())
...
function handleStopButtonClick() {
	...
	recordingStatus.textContent = geti18n('Rika.config.recording_stopped')
}
...
cameraButton.addEventListener('click', async () => {
	...
	try {
		...
		uploadStatus.textContent = geti18n('Rika.config.camera_on')
	}
	catch (error) {
		console.error('Error accessing camera:', error)
		uploadStatus.textContent = geti18n('Rika.config.cannot_access_camera', { errorMessage: error.message })
	}
})
...
captureButton.addEventListener('click', () => {
	...
	canvas.toBlob(blob => {
		...
		photoStatus.textContent = geti18n('Rika.config.photo_captured_click_to_save')
...
function handlePhotoInputChange() {
	...
	if (selectedPhotoFile) {
		...
		reader.onload = e => {
			...
			imagePreview.classList.remove('hidden')
			photoStatus.textContent = geti18n('Rika.config.new_image_selected_click_to_save')
			photoStatus.classList.remove('cursor-pointer', 'hover:underline')
			photoStatus.onclick = null
		}
...
savePhotoButton.addEventListener('click', async () => {
	if (!selectedPhotoFile) {
		uploadStatus.textContent = geti18n('Rika.config.please_select_or_take_photo_first')
		return
	}
	const success = await saveFile('vars/master-photo-reference.png', selectedPhotoFile, uploadStatus)
	if (success) {
		photoStatus.textContent = geti18n('Rika.config.photo_updated')
		loadPreview('vars/master-photo-reference.png', imagePreview, photoStatus, 'photo')
