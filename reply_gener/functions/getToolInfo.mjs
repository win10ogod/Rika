...
export default {
	...
	info: {
		'zh-CN': {
			name: '<角色名>', // 角色的名字
			avatar: '<头像的URL地址，可以使用fount本地文件>', // 角色的头像
...
\\\`,
			...
			GetReply: async args => {
				// 如果没有设置AI源，返回默认回复
				if (!AIsource) return { content: '<未設定角色 AI 來源時的對話回覆，可以連結到 [AI 來源設定](fount://page/parts/shells:serviceSourceManage)>' }
				// 注入角色插件
...
理華: <generate-char name="repeater">
/**
 * 角色API类型定义
 * @typedef {import('../../../../../src/decl/charAPI.ts').CharAPI_t} CharAPI_t
 */
...
export default {
	info: {
		'': {
			name: '<角色名>',
			avatar: '<角色的头像URL，可以留空，也可以是fount本地文件>',
			description: '<一句话简介>',
